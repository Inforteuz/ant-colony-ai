...
async def serve_agent_screenshot(path: str):
    """
    Agentlar tomonidan olingan screenshot faylini serve qiladi.
    Faqat loyihalar ichidagi `.ant_colony_shots/` papkasidagi PNG'lar ruxsat etiladi.
    Xavfsizlik: path traversal himoyasi — fayl faqat PROJECTS_BASE_DIR ostida bo'lishi shart.
    """
    ...
    try:
        ...
        # Loyihalar papkasi ostida bo'lishi shart
        target.relative_to(base)
        # Faqat .ant_colony_shots ostidagi PNG
        if ".ant_colony_shots" not in target.parts:
            raise HTTPException(status_code=403, detail="Faqat .ant_colony_shots papkasidagi fayllar")
        if target.suffix.lower() != ".png":
            raise HTTPException(status_code=403, detail="Faqat PNG fayllar")
